...
def category_scan(category,urls):
    if category in ['Shopping','Finance','Autos_and_Vehicles']:
        basic_scan(urls,1)
        print(f"function performed for {category} {urls}")
    elif category in ['News_and_Media','Books_and_Literature','Law_and_Government','Reference','Career_and_Education','Science','Adult (Sensitive)','Gambling (Sensitive)']:
        basic_scan(urls,2)
        print(f"function performed for {category} {urls}")
